...
AtomVecCCFull::AtomVecCCFull(LAMMPS *lmp, int narg, char **arg) : 
  AtomVec(lmp, narg, arg)
{
  ...
  size_border = 12+CTYPES;
  size_data_atom = 7;//+CTYPES;
  size_data_vel = 4;
  xcol_data = 5;
...
void AtomVecCCFull::grow(int n)
{
  ...
  bond_length = atom->bond_length = 
    memory->grow_2d_double_array(atom->bond_length,nmax,atom->bond_per_atom,
        "atom:bond_length");
  ...
  angle_area = atom->angle_area = 
    memory->grow_2d_double_array(atom->angle_area,nmax,atom->angle_per_atom,
                                          "atom:angle_area");
...
void AtomVecCCFull::reset_special()
{
  special = atom->special;
}
...
void AtomVecCCFull::copy(int i, int j)
{
  int k;
...
int AtomVecCCFull::pack_comm(int n, int *list, double *buf,
...
int AtomVecCCFull::pack_comm_one(int i, double *buf)
{
  buf[0] = v[i][0];
  buf[1] = v[i][1];
  buf[2] = v[i][2];
...
void AtomVecCCFull::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecCCFull::unpack_comm_one(int i, double *buf)
{
  v[i][0] = buf[0];
  v[i][1] = buf[1];
  v[i][2] = buf[2];
...
int AtomVecCCFull::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecCCFull::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecCCFull::pack_border(int n, int *list, double *buf,
			     int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecCCFull::pack_border_one(int i, double *buf)
{
  buf[0] = q[i];
  buf[1] = molecule[i];
  buf[2] = v[i][0];
...
void AtomVecCCFull::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecCCFull::unpack_border_one(int i, double *buf)
{
  q[i] = buf[0];
...
int AtomVecCCFull::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecCCFull::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecCCFull::size_restart()
{
  int i;
  ...
  int n = 0;
  for (i = 0; i < nlocal; i++)
    n += 17 + CTYPES + 3*num_bond[i] + 5*num_angle[i] +
...
int AtomVecCCFull::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecCCFull::unpack_restart(double *buf)
{
  int k;
...
void AtomVecCCFull::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecCCFull::data_atom(double *coord, int imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  ...
  for(int k = 0; k < CTYPES ; k++)
    T[nlocal][k] = 0.0;//atof( values[7+k] );
...
int AtomVecCCFull::data_atom_hybrid(int nlocal, char **values)
{
  molecule[nlocal] = atoi(values[1]);
  q[nlocal] = atof(values[3]);
...
double AtomVecCCFull::memory_usage()
{
  double bytes = 0.0;
