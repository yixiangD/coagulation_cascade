...
void DumpAtom::init()
{
  if (image_flag == 0 || image_flag == 2) size_one = 5;
  else size_one = 8;
  ...
  if (format_user) {
    ...
    char *str;
    if (image_flag == 0 || image_flag == 2) str = (char *) "%d %d %g %g %g";
  ...
  else if (scale_flag == 0 && image_flag == 2)
    pack_choice = &DumpAtom::pack_noscale_do_image;

  if (binary) write_choice = &DumpAtom::write_binary;
  else if (image_flag == 0 || image_flag == 2) write_choice = &DumpAtom::write_noimage;
...
int DumpAtom::modify_param(int narg, char **arg)
{
  if (strcmp(arg[0],"scale") == 0) {
    ...
    else if (strcmp(arg[1],"do") == 0) image_flag = 2;
    else error->all("Illegal dump_modify command");
    return 2;
  }
  return 0;
...
int DumpAtom::pack_noscale_do_image()
{
  double xx[3];
  int *tag = atom->tag;
  int *type = atom->type;
