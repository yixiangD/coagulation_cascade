...
PairMorseDPD::~PairMorseDPD()
{
  if (allocated) {
    ...
    memory->destroy_2d_double_array(r_act);
...
void PairMorseDPD::compute(int eflag, int vflag)
{
  ...
  int newton_pair = force->newton_pair;
	...
	if (t_step < 100000) return;		// Alireza: hardcoded for the flow to develop 
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (r < r_act[itype][jtype] && q[j] == 1.0)
        if (itype == 2 && q[i] == 0.0){
          nnn = n_act*(random->uniform() + 0.5);
          q[i] = -1.0 - nnn;         
				}

      if (r < cut[itype][jtype] && q[i] >= 1.0 && q[j] >= 1.0 && q[j] <= 100.0 && modf(q[j],&intpart) == 0.0){
        q[i] = 1.0; 
		  	dr = r - r0[itype][jtype];
  			...
  			fpair = factor_lj * (morse1[itype][jtype] * (dexp*dexp - dexp) - shift[itype][jtype]) / r;
...
void PairMorseDPD::allocate()
{
  ...
  r_act = memory->create_2d_double_array(n+1,n+1,"pair:r_act");
...
double PairMorseDPD::init_one(int i, int j)
{
  ...
  if (offset_flag) {
    ...
    offset[i][j] = d0[i][j] * (exp(2.0*alpha_dr) - 2.0*exp(alpha_dr));
		shift[i][j] = morse1[i][j] * (exp(2.0*alpha_dr) - exp(alpha_dr));
  } else {
		offset[i][j] = 0.0;
		shift[i][j] = 0.0;
	}
...
void PairMorseDPD::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
	...
	fwrite(&r_act[i][j],sizeof(double),1,fp);
      }
    }
}
...
void PairMorseDPD::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
	...
	if (me == 0) {
	  ...
	  fread(&r_act[i][j],sizeof(double),1,fp);
	}
	MPI_Bcast(&d0[i][j],1,MPI_DOUBLE,0,world);
	MPI_Bcast(&alpha[i][j],1,MPI_DOUBLE,0,world);
	MPI_Bcast(&r0[i][j],1,MPI_DOUBLE,0,world);
	MPI_Bcast(&cut[i][j],1,MPI_DOUBLE,0,world);
	MPI_Bcast(&r_act[i][j],1,MPI_DOUBLE,0,world);
      }
    }
}
