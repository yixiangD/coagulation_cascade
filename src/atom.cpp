...
Atom::~Atom()
{
  ...
  memory->destroy_2d_double_array(T);
  memory->destroy_2d_double_array(Q);
  ...
  memory->destroy_2d_double_array(bond_length);
  ...
  memory->destroy_2d_double_array(angle_area); 
...
void Atom::init()
{
  ...
  if (molecular){
    ...
    for (i=0; i<nlocal; i++)
      if (nnmol < molecule[i]) 
        nnmol = molecule[i];
    MPI_Allreduce(&nnmol,&n_mol,1,MPI_INT,MPI_MAX,world);

    if (n_mol > 0) {
      atoms_in_mol = (int *) memory->smalloc(n_mol*sizeof(int),"atom:atoms_in_mol");
      atmol = (int *) memory->smalloc(n_mol*sizeof(int),"atom:atmol");
      for (i=0; i<n_mol; i++){ 
        atoms_in_mol[i] = 0;
        atmol[i] = 0;
      }
      ...
      for (i=0; i<nlocal; i++)
        if (molecule[i]) 
          atmol[molecule[i]-1] +=1;
      MPI_Allreduce(atmol,atoms_in_mol,n_mol,MPI_INT,MPI_SUM,world);
...
void Atom::data_bonds(int n, char *buf)
{
  ...
  for (int i = 0; i < n; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    sscanf(buf,"%d %d %d %d %lg",&tmp,&itype,&atom1,&atom2, &l0tmp);
...
void Atom::data_angles(int n, char *buf)
{
  ...
  for (int i = 0; i < n; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    sscanf(buf,"%d %d %d %d %d %lg",&tmp,&itype,&atom1,&atom2,&atom3,&a0tmp);
    if (atom1 <= 0 || atom1 > map_tag_max || 
	atom2 <= 0 || atom2 > map_tag_max || 
