...
enum{TYPE,TYPE_FRACTION,MOLECULE,
       ...
       BOND,ANGLE,DIHEDRAL,IMPROPER,CONCEN};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
	...
	error->all("Invalid value in set command");
      topology(IMPROPER);
      iarg += 2;
    } else if (strcmp(arg[iarg],"concen") == 0){ //Alireza: initial concentrations
      if (iarg+2 > narg) error->all("Illegal set command");
    	char fname[FILENAME_MAX];
    	sprintf(fname,arg[iarg+1]);
    	par = (double *) memory->smalloc(CTYPES*sizeof(double),"set:par");
    	...
    	if (comm->me == 0){
      	...
      	if(file_read.fail())
        	error->all("set: Initial concentration file should be prepared in this folder.");
    	...
    	MPI_Bcast(par,CTYPES,MPI_DOUBLE,0,world);
			set(CONCEN);
			iarg += 2;
		} else error->all("Illegal set command");    
