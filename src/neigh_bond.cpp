...
/*  if(update->ntimestep % 100 == 0)
  {
    ...
    {
      if(type[i] == 2)
        printf("it is %ld with %ld bonds in proc %d\n", tag[i], num_bond[i], me);
    }
  }*/
  MPI_Barrier(world);
  for (i = 0; i < nlocal; i++)
	...
	if (nbondlist == maxbond) {
	  ...
	  bondlist = memory->grow_2d_int_array(bondlist,maxbond,3,
					       "neighbor:bondlist");
    bondlist_length = (double *) memory->srealloc(bondlist_length,maxbond*sizeof(double),
                 "neighbor:bondlist_length");
	}
  ...
  for (i = 0; i < nlocal; i++)
	...
	if (nanglelist == maxangle) {
	  ...
	  anglelist = memory->grow_2d_int_array(anglelist,maxangle,4,
						  "neighbor:anglelist");
          anglelist_area = (double *) memory->srealloc(anglelist_area,maxangle*sizeof(double),
                                                  "neighbor:anglelist_area");
	}
