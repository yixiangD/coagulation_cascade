...
void CreateAtoms::add_random()
{
  ...
  double x[3],lamda[3];
  double *coord;
  double sublo[3],subhi[3],boxlo[3],boxhi[3];
  ...
  if (triclinic == 0) {
    ...
    sublo[1] = domain->sublo_lamda[1]; subhi[1] = domain->subhi_lamda[1];
    sublo[2] = domain->sublo_lamda[2]; subhi[2] = domain->subhi_lamda[2];
    boxlo[0] = domain->boxlo_lamda[0]; boxhi[0] = domain->boxhi_lamda[0];
    boxlo[1] = domain->boxlo_lamda[1]; boxhi[1] = domain->boxhi_lamda[1];
    boxlo[2] = domain->boxlo_lamda[2]; boxhi[2] = domain->boxhi_lamda[2];
  }
  ...
  for (int i = 0; i < nrandom; i++) {
    while (1) {
      x[0] = xlo + random->uniform() * (xhi-xlo);
      x[1] = ylo + random->uniform() * (yhi-ylo);
      x[2] = zlo + random->uniform() * (zhi-zlo);
      if (domain->dimension == 2) x[2] = zmid;

      valid = 1;
      if (nregion >= 0 && domain->regions[nregion]->match(x[0],x[1],x[2]) == 0)
        valid = 0;
      if (triclinic) {
