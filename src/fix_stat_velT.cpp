...
FixStatVelT::FixStatVelT(LAMMPS *lmp, int narg, char **arg) :
  FixStat(lmp, narg, arg)
{
  int i, j, k;
  num = memory->create_3d_double_array(nx,ny,nz,"fix_stat_velT:num");
  vx = memory->create_3d_double_array(nx,ny,nz,"fix_stat_velT:vx");
  vy = memory->create_3d_double_array(nx,ny,nz,"fix_stat_velT:vy");
  vz = memory->create_3d_double_array(nx,ny,nz,"fix_stat_velT:vz");
  aT = memory->create_3d_double_array(nx,ny,nz,"fix_stat_velT:aT");
  for (i=0; i<nx; i++)
    for (j=0; j<ny; j++)
      for (k=0; k<nz; k++){
        num[i][j][k] = 0.0;
...
void FixStatVelT:: write_stat(int step)
{
  int i, j, k, l;
  double x, y, z;
  int total = nx*ny*nz;
